...
如果你不喜欢默认的键盘对应，您可以禁用单独的键盘快捷键，disabled_keymap_actions偏好Emmet.sublime设置文件。
...
这并不是说，如果你禁用了任何行动，像这样和你创建自己的键盘快捷键，你不应该使用emmet_action_enabled.ACTION_NAME情况下，因为这是关键，禁用行动。
“救命啊！我的片段不工作了HTML / CSS文件！“

默认情况下，埃米特覆盖Tab键的行为，并扩大自己的缩写，而不是原生代码片段。您可以禁用此功能，在用户首选项（添加“disable_tab_abbreviations” 设置-设置用户文件），使用Ctrl + E组合键或按Ctrl + Alt + Enter键，扩大埃米特abbeviations或移动您的片段埃米特这里所描述的。我调查的可能性，以扩大本地的片段通过埃米特Tab键处理程序。
